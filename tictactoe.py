...
class TicTacToeState(game.GameState):
    ...
    def winner(self):
        ...
        for i in range(3):
            if self._checkelems(state[3 * i], [state[3 * i + e] for e in range(3)]): #check the columns
                return state[3 * i]
            if self._checkelems(state[i], [state[3 * e + i] for e in range(3)]): #check the rows
                return state[i]
        # Check diagonals
...
class TicTacToeClient(game.GameClient):
    ...
    def _nextmove(self, state):
        for i in range(9):
            if state._state['visible'][i] == None:
                    if (((i<8) and state._state['visible'][i+1] == self._playernb) or ((i<5) and state._state['visible'][i+3] == self._playernb) or ((i<4) and state._state['visible'][i+4]==self._playernb)):
                        return str (i)
                    elif (i>0) and (state._state['visible'][i-1] == self._playernb):
                        return str (i)
                    elif (i>2) and (state._state['visible'][i-3] == self._playernb):
                        return str (i)
                    elif (i>3) and (state._state['visible'][i-4] == self._playernb):
                        return str (i)
                
        return str(state._state['visible'].index(None)) #renvoie la première position où se trouve une case vide => triste
